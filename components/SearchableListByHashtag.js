...
class SearchableListByHashtag extends Component {
  ...
  /**
   * When the SearchScreenByHashtag is mounted, we'll call makeRemoteRequest() to get some data.
   */
  componentDidMount() {
  ...
  /**
   * This function goes and fetches some data using API(https://am-twitter-scrape.herokuapp.com/hashtags/Python?pages_limit=3&wait=0).
  ...
  makeRemoteRequest = () => {
    ...
    const url = `https://am-twitter-scrape.herokuapp.com/hashtags/Python?pages_limit=3&wait=0`;
  ...
  /**
   * this function will change the index of hashtags.
   * it will be calling inside searchFilterFunction().
   */
  ...
  searchFilterFunction = text => {
    ...
    const newData = this.resHolder.filter(item => {
      ...
      for (let i = 0; i < item.hashtags.length; i++) {
        const check = patt.test(item.hashtags[i]);
        if ( check ) {
          item.hashtags = this.array_move(item.hashtags, i, 0);
          return check;
        }
      }
...
export default SearchableListByHashtag;
